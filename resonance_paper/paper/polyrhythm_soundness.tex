...
\title{\bfseries No universal estimator of \textit{n:m} phase coupling:\\
complementary metric blind spots and a validated cross-signal detector}
...
organizing results. First, \textbf{no single estimator is universal}: the \textit{n:m} phase-locking
value (PLV) is the most sensitive detector on clean, unimodal locks and at low signal-to-noise, but is
blind by construction to \emph{multimodal} locks, where entropy- and mutual-information indices recover
the coupling it misses --- so sound measurement needs a metric \emph{panel}, not a single statistic.
Second, \textbf{the harmonic confound is a matter of scope}: within one non-sinusoidal signal every
technique false-positives and no standard surrogate fully removes it, so \textit{n:m} coupling is soundly
interpretable only between independent sources. Along the way we isolate the estimation choices that
...
four choices --- convention, phase estimator, ratio coverage, weighting --- are decisive; no single
technique is universal; and \textit{n:m} coupling is trustworthy only between independent sources. We turn
each answer into a concrete recommendation and ship a validated cross-signal detector.
...
defaults can be not merely weak but \emph{directionally wrong}: a plausible pipeline recovered almost none
of a textbook polyrhythm and, under a swapped ratio convention, scored genuine coupling below chance ---
and the failure was invisible without ground truth. Second, there is \emph{no

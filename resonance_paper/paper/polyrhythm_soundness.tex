...
genuine polyrhythms are present and recoverable, and --- in sleep EEG --- that the new techniques recover
a genuine, surrogate-controlled, state-dependent \textit{n:m} phase-coupling spectrum (while
inter-regional coupling stays at the noise floor). We then show that the \emph{default} settings of a
representative estimator recover almost none of a clean 2:3 lock (a detection margin of $0.02$ against an
oracle's $0.79$) and, under a swapped ratio convention, \textbf{anti-detect} --- scoring genuine coupling
below chance --- and we trace and correct four independent defects. With the corrected estimator no single technique is best:
...
\subsection{Default settings fail on a clean polyrhythm}
...
($\Delta=0.79$). The representative estimator's \emph{default} configuration recovers almost none of it:
the coupled--uncoupled margin collapses to $\Delta=0.02$ (Figure~\ref{fig:defaults}A). Four
...
\caption{\textbf{Default settings fail on a known lock.}
(A) Detection margin (coupled $-$ uncoupled) for a clean 2:3 lock: the default configuration collapses to
near zero ($\Delta=0.02$), the corrected configuration improves it, and only the oracle separates cleanly
($\Delta=0.79$).

...
| 7 | [study7_coupled_oscillators.py](resonance_paper/study7_coupled_oscillators.py) | Generative coupling: harmonically-coupled phase oscillators; locking threshold K\* vs ratio complexity | generative | **K\*=3.3/8.7/23.5 for 2:3/3:4/4:5 (ρ=+1.0): complex ratios lock later**; PC tracks the emergent lock, H blind |
...
| 21 | [study21_connectivity.py](resonance_paper/study21_connectivity.py) | Multichannel: does cross-resonance *connectivity* recover a planted coupled cluster? | synthetic 8-ch network | H/PC/R matrices recover the cluster (H 0.74, PC 0.80, R≈PC 0.81); reducer is broadband-biased so recovery is overlap-driven — surrogate-z isolates pure phase (honest capability demo) |
...
| 23 | [study23_R_justification.py](resonance_paper/study23_R_justification.py) | Is R = H·PC justified? which combine rule? | synthetic | conjunction beats **both** factors under independence (≈0.99 vs H/PC 0.75; disjunction max 0.66); phase-scramble → R adds the phase gate H lacks (AUC 1.0 vs H 0.50); product chosen for log-additivity (min ties) |
| 24 | [study24_operating_characteristics.py](resonance_paper/study24_operating_characteristics.py) | Operating range, null calibration, scaling? | synthetic | H detection robust to ≈−18 dB, coupling threshold ≈−12 dB; cross PC_z null FPR≈0.09 @ α=.05 (mildly anti-conservative); runtime scales with n_freqs, flat in signal length |
| 25 | [study25_baselines.py](resonance_paper/study25_baselines.py) | How do H/PC compare to established measures? | synthetic | PC_z matches oracle raw n:m PLV and is **~6 dB more robust at low SNR**; H matches HNR at usable SNR (HNR more noise-robust below −24 dB) |
...
ratio complexity** (K\*=3.3/8.7/23.5 for 2:3/3:4/4:5; ρ(K\*,n·m)=+1.0) — the
